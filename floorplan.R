...
# added a plain version without logos/text
g <- rasterGrob(readPNG("floorplan_plain.png"), interpolate=TRUE) 
p <- ggplot(rooms, aes(x = x, y = y)) +
  xlim(0,255) + ylim(0,156) +
  annotation_custom(g, xmin = -Inf, xmax = Inf, ymin = -Inf,  ymax = Inf) +
  geom_text(aes(label = first), hjust = 0, size = 1.6, nudge_x = 1, nudge_y = 6, na.rm = TRUE) +
...
ggsave("FindMyTutor_plain.png", p, width=25.5, height = 15.6, units='cm')
